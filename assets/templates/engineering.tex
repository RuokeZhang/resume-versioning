...
% classic.tex (references/INTERFACE.md), so one content composition renders
% under either. Input this from a driver, after the driver has set its identity
...
% Font. A driver selects one by name before \input-ing this template:
%   \newcommand{\ResumeFont}{carlito}
% It is input here rather than by the driver because font packages must load
% after fontenc; loading one earlier changes the encoding setup and silently
% shifts the metrics.
\providecommand{\ResumeFont}{sourcesans}
\input{fonts/\ResumeFont.tex}
